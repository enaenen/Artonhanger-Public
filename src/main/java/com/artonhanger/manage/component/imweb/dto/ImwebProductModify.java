...
@NoArgsConstructor
public class ImwebProductModify {
    ...
    // TODO 매핑 로직 따로 작성하기..?
    public ImwebProductModify(Artwork artwork) {
    ...
    public ImwebProductModify(Artwork artwork, ArtworkModifyDto artworkModifyDto) {
        this.isEdition = artworkModifyDto.isEdition();
        categorySet(artwork.getCategories());
        materialSet(artwork.getMaterials());
        artworkImageSet(artwork.getArtworkImages());
        artDeliveryConvert(artwork.getShipping());
        ...
        this.stockUnlimit = false;
        this.imwebId = artwork.getImwebId();
        this.stockAmount = artworkModifyDto.getStockAmount();
    }
    ...
    private void setContents(List<ArtworkImage> artworkImage) {
        ...
        for (ArtworkImage image : artworkImage) {
            ...
            sb.append(convertSpecialStringInDescription(image.getDescription()));
            sb.append("<br>");
            sb.append("<br>");
        }
    ...
    private String convertSpecialStringInDescription(String description) {
    ...
    private void categoryConvert(CategoryEnum categoryEnum) {
        ...
        if (categoryEnum.equals(CategoryEnum.SCULPTURE))
            categories.add(CategoryWebEnum.SCULPTURE.getCategoryCode());
    }
